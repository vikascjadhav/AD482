...
public class TumblingWindows extends StreamProcessor {
    ...
    void onStart(@Observes StartupEvent startupEvent) {
        ...
        ObjectMapperSerde<PotentialCustomersWereDetected> customersEventSerde = new ObjectMapperSerde<>(
                PotentialCustomersWereDetected.class);

        // TODO: Build the stream topology

        builder.stream(POTENTIAL_CUSTOMERS_TOPIC, Consumed.with(Serdes.String(), customersEventSerde)).groupByKey()
                .windowedBy(TimeWindows.of(Duration.ofSeconds(WINDOW_SIZE)).grace(Duration.ofSeconds(12L))
                .grace(Duration.ofSeconds(0))).count().toStream().print(Printed.toSysOut());
